<template>
	<div class="post-votes">
	    <i class="icon feather icon-thumbs-up" :disabled="vote_status" :class="{upvoted: posts.data.upvoted}" @click="upvote(posts.data.id)" ></i>
	    <span id="raiting" :style="{color: colorStatus}" style="font-family: 'Roboto',Tahoma; cursor: default;">{{ posts.data.votes }}</span>
	    <i class="icon feather icon-thumbs-down" :disabled="vote_status" :class="{downvoted: posts.data.downvoted}" @click="downvote(posts.data.id)"></i>
	</div>
</template>
